"""
commodities_optimizer.py  –  Commodities Portfolio Optimizer
=======================================================
Ablegen: dashboard/pages/commodities_optimizer.py
Starten: streamlit run dashboard/app.py

Universum:  Commodities (asset_class == "Commodities")
Portfolios: Equal Weight · Max Utility (λ) · Min CVaR 95%
Constraints: keine – freie Optimierung über alle Commodities
"""
...
# PAGE CONFIG + STYLING
# ─────────────────────────────────────────────
st.set_page_config(page_title="Commodities Optimizer", page_icon="🔵",
...
# Filter: nur Commodities die auch Preisdaten haben
BOND_ETF_UNIVERSE = [
    a for a in PM_ALL.columns
    if ameta(a).get("asset_class","").lower() == "commodities"
]

if not BOND_ETF_UNIVERSE:
    st.error("No Commodities found in price data."); st.stop()
...
with st.sidebar:
    st.markdown("## 🟡 Commodities Optimizer")
    st.markdown('<p style="font-size:.75rem;color:#CBD5E1;margin-top:-.4rem;">'
                "Commodities · No constraints</p>", unsafe_allow_html=True)
    st.markdown("---")
...
# ─────────────────────────────────────────────
if len(sel_assets) < 2:
    st.warning("Please select at least 2 Commodities."); st.stop()
...
with tab1:
    ...
    st.markdown('<span class="chip">Individual Commodity Performance</span>',
                unsafe_allow_html=True)

    f = mfig("Cumulative Returns – all Commodities", h=420)
...
# ══════════════════════════════════════════════
with tab3:
    f = mfig("Efficient Frontier – Commodities Universe (unconstrained)", h=540,
...
with tab4:
    ...
    f.update_layout(**_lo("Correlation Matrix – Commodities (monthly returns)", h=460))
    st.plotly_chart(f, use_container_width=True)
...
st.markdown(
    ...
    f'Commodities Universe · {params["lookback_months"]} M lookback · '
