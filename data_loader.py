...
# EXCEL EINLESEN
# ─────────────────────────────────────────────
def _read_bloomberg_excel(filepath):  # type: (Path) -> Tuple[str, pd.DataFrame]
...
def _load_raw(data_dir):  # type: (Path) -> Dict[str, pd.DataFrame]
    raw = {}
...
# ─────────────────────────────────────────────
def _to_monthly(prices_daily):  # type: (pd.DataFrame) -> pd.DataFrame
    """
    Resamplet tägliche Kurse auf den letzten Handelstag je Monat.
    """
    return prices_daily.resample("M").last().dropna()


# ─────────────────────────────────────────────
# KENNZAHLEN  (monatliche Renditen als Input)
# ─────────────────────────────────────────────
def _metrics(r):  # type: (pd.Series) -> Dict[str, float]
    """
...
# HAUPTFUNKTION
# ─────────────────────────────────────────────
def load_portfolio(data_dir=None):  # type: (Optional[Union[str, Path]]) -> Dict[str, object]
    """
    ...
    for name in POSITIONS_EUR:
        if name in raw:
            ...
            print("  {}  '{}' nicht gefunden. Verfügbare Namen: {}".format("⚠️", name, list(raw.keys())))
    ...
    if len(returns) > 0:
        print(
            "\n  📅  Monatliche Renditen: {} Monate ({} – {})".format(
                len(returns),
                returns.index[0].strftime("%b %Y"),
                returns.index[-1].strftime("%b %Y")
            )
        )
    else:
        print("\n  ⚠️  Keine monatlichen Renditen berechnet.")

    # ── Gewichte ──────────────────────────────────────────────────────────
    avail = dict((k, v) for k, v in POSITIONS_EUR.items() if k in prices_daily.columns)
    total_v = sum(avail.values())
    weights = pd.Series(dict((k, v / total_v) for k, v in avail.items()))
